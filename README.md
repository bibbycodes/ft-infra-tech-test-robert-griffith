...
This will deploy the app to paws and provide you with a URL from which you can access the API.
...
1000.0
```
Here we have made an acount object and initialzed it a starting balance of 1000. If the starting balance is not supplied, the account will be initialzed with a balance of 0.
...
1500.0
```
If we want to print a statement, we simply pass the account abject to the statement class and print it.
```shell
...
```
`add_transaction` also accepts a third optional argument that specifies the transaction date. If this argument is not supplied, then the transaction date is set to today. The date argument can be supplied in the format of dd/mm/yyy, dd-mm-yyyy or as a timestamp.
```shell
